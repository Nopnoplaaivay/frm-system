...
class PortfolioPerformance(StrategyInterface):
    def __init__(self, price_matrix):
        self.price_matrix = price_matrix
        self.symbols = price_matrix.columns

    def compute(self):
        returns = self.price_matrix[self.symbols].pct_change().dropna()
        ...
        weights_df = pd.DataFrame(weights_record, columns=self.symbols)

        return results_df, weights_df

    def render_chart(self):
        results_df, weights_df = self.compute()
        ...
        axes[0, 1].set_title("Treynor Ratio", fontsize=12)
        ...
        axes[1, 0].set_title("Jensen's Alpha", fontsize=12)
        ...
        axes[1, 1].set_title("Sortino Ratio", fontsize=12)
        ...
        for i, metric in enumerate(metrics):
            ...
            axes[i].set_title(f"{best_metric}", fontsize=12)
            axes[i].set_xlabel("Asset", fontsize=12)
            axes[i].set_ylabel("Weight", fontsize=12)
            axes[i].set_xticklabels(self.symbols, rotation=90 , fontsize=12)
